...
impl GitHubClient {
    pub async fn fetch_prs(&self) -> Result<PrState> {
        let authored = self.search_prs(
            &format!("author:{} type:pr state:open", self.username)
        ).await?;

        let review_requested = self.search_prs(
            &format!("review-requested:{} type:pr state:open", self.username)
        ).await?;

        Ok(PrState { authored, review_requested })
    }

    async fn search_prs(&self, query: &str) -> Result<Vec<PrInfo>> {
        let results: serde_json::Value = self.octocrab.get(
            "/search/issues",
